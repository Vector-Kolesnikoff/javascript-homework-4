...
function wordsList(str, subStr) { // Создал функцию с параметрами.
  var re = new RegExp('\\.|,|!|\\?|;|:|"', 'gui'); // Создал переменную, а в ней конструктор RegExp, который создаёт объект регулярного выражения для сопоставления текста с шаблоном.
  var arr = str // Собъявил переменную arr, которая является строкой, и к ней идёт набор методов:
    .replace(re, '') // заменяет все элементы, относящиеся к переменной re на пустое значение '';
    .toLowerCase() // переводит текст строки в нижний регистр;
    .split(' ') // разделяет строку на элементы массива посредством пробелов;
    .filter((arrItem) => // метод filter() создаёт новый массив со всеми элементами, прошедшими проверку, задаваемую в передаваемой функции.
      arrItem.indexOf(subStr) > -1 // Метод indexOf() возвращает первый индекс, по которому данный элемент может быть найден в массиве или -1, если такого индекса нет (в данном случае, всё что больше -1).
    );

  var set = new Set(); // Объявлена переменная set, метод (или конструктор?) Set - коллекция для хранения множества значений, причем каждое значение может встречаться лишь один раз..

    arr.forEach((arrItem) => { // Метод forEach() выполняет указанную функцию один раз для каждого элемента в массиве.
      set.add(arrItem); // Метод add() добавляет новый элемент с заданным значением в конец объекта Set.
    }); // Вся последняя функция (arr) начиная с объявления set - это для того чтобы вернуть коллекцию уникальных значений, а всё, что выше - дополнительные условия.
  return set;
}; 
...
function getLocalDate(date, isSeconds, isISO){
  ...
  if (isISO) str = year + '-' + month + '-' + days + ', ' + hours + ':' + minutes;
  else str = days + '.' + month + '.' + year + ', ' + hours + ':' + minutes;
  if (isSeconds) str = str + ':' + seconds;
  return str;
}
...
console.log(getLocalDay('2019-07-14')); // 7
...
function getDateAgo (date, days) {
  ...
  return newDate.toLocaleString().replace(/(\d.*),\s+(\d.*)/gu, '$1');
}
